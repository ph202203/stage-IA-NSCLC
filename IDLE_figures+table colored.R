...
# Prediction accuracy (AUC and KM curves) =======
...
# heatmap ======
data4<- read.csv("Heatmap CT+HE data.csv")
data1<- data4[, c(-1,-2)]
data1<- as.matrix(data1)
data1<- t(data1)
col.pfs<- rep("red", nrow(data4))
col.pfs[data4$ind.lung.pfs==0]<- "green"
...
par(mfrow=c(2,3))
# prediction from LDCT features only
{
# 5- and 10-year AUC 
{dataset<- texture.roi
set.seed(123)
 y5.ct<- survivalROC(Stime=dataset$days.pfs,
		status=dataset$ind.lung.pfs, marker=dataset$IDLE.ct,
...
# 5-year
results<- PPV.NPV(y5.ct, dataset$IDLE.ct)
results<- results[-1,]
results<- results[with(results, order(sensitivity)),]
results<- results[results$sensitivity>0.6,]
results<- results[results$sensitivity<0.95,]
...
# 10-year
results<- PPV.NPV(y10.ct, dataset$IDLE.ct)
results<- results[-1,]
results<- results[with(results, order(sensitivity)),]
results<- results[results$sensitivity>0.6,]
results<- results[results$sensitivity<0.95,]
...
plot(results.5$sensitivity, results.5$PPV,
   ...
   value from CT features", ylim=c(0.25,0.45), type="l", lwd=2,
  xlab="Sensitivity", ylab="", col="black")
...
plot(results.5$sensitivity, results.5$NPV,
   ...
   value from CT features", ylim=c(0.72,1), type="l", lwd=2,
  xlab="Sensitivity", ylab="", col="black")
...
# prediction from tissue features only
{
# 5- and 10-year AUC 
{dataset<- texture.roi
set.seed(123)
 y5.roi<- survivalROC(Stime=dataset$days.pfs,
		status=dataset$ind.lung.pfs, marker=dataset$IDLE.tissue,
		predict.time=5*365, method="KM")
 y10.roi<- survivalROC(Stime=dataset$days.pfs,
		status=dataset$ind.lung.pfs, marker=dataset$IDLE.tissue,
		predict.time=10*365, method="KM")

plot(y5.roi$FP, y5.roi$TP, type="l", xlim=c(0,1), ylim=c(0,1),
	xlab="1-specificity", ylab="sensitivity", 
  col="black",lwd=2,
	cex.main=1, cex.lab=1,cex.axis=1,
  main="Time-dependent ROC from\ntissue features")
lines(y10.roi$FP, y10.roi$TP,lwd=2, col="red") 
lines(c(0,1),c(0, 1), lty=2, col="grey")
text(0.7,0.25,paste0("5-year AUC=", round(y5.roi$AUC,3)),
  col="black",cex=0.9)
text(0.7,0.15,paste0("10-year AUC=", round(y10.roi$AUC,3)),
  col="red",cex=0.9)
}
# 5- and 10-year time dependent 
# PPV, NPV conditioning on sensitivity
...
dataset<- texture.roi
set.seed(123)
y5.roi<- survivalROC(Stime=dataset$days.pfs/365,
		status=dataset$ind.lung.pfs, marker= dataset$IDLE.tissue,
		predict.time=5, method="NNE", span = 0.07)
set.seed(123)
y10.roi<- survivalROC(Stime=dataset$days.pfs/365,
		status=dataset$ind.lung.pfs, marker= dataset$IDLE.tissue,
		predict.time=10, method="NNE", span = 0.07)
# 5-year
results<- PPV.NPV(y5.roi, dataset$IDLE.tissue)
results<- results[-1,]
results<- results[with(results, order(sensitivity)),]
results<- results[results$sensitivity>0.6,]
results<- results[results$sensitivity<0.95,]
...
# 10-year
results<- PPV.NPV(y10.roi, dataset$IDLE.tissue)
results<- results[-1,]
results<- results[with(results, order(sensitivity)),]
results<- results[results$sensitivity>0.6,]
results<- results[results$sensitivity<0.95,]
...
plot(results.5$sensitivity, results.5$PPV,
    main="Time-dependent positive predictive
   value from tissue features", ylim=c(0.25,0.45), type="l", lwd=2,
  xlab="Sensitivity", ylab="", col="black")
lines(results.10$sensitivity, results.10$PPV,
  col="red",lwd=2)
legend("topright", c("year 5","year 10"),
...
plot(results.5$sensitivity, results.5$NPV,
    main="Time-dependent negative predictive
   value from tissue features", ylim=c(0.72,1), type="l", lwd=2,
  xlab="Sensitivity", ylab="", col="black")
lines(results.10$sensitivity, results.10$NPV,
  col="red",lwd=2)
legend("topright", c("year 5","year 10"),
